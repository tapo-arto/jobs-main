...
$base_url = admin_url('admin.php?page=tjobs-settings');
if ($only_changes === 1) {
...
<div class="wrap">
        ...
        <table class="form-table">
            ...
            <tr>
                ...
                <td>
                    ...
                    <p class="description"><?php esc_html_e( 'Käytetään, jos automaattinen kohdistus ei löydä sopivaa infopakettia.', 'tapojarvijobs' ); ?></p>
                </td>
            </tr>
        </table>
        <p>
            <button type="submit" name="tjobs_save_settings" class="button button-primary">Tallenna</button>
            <button type="submit" name="tjobs_force_import" class="button button-secondary">Pakota tuonti</button>
        </p>
    </form>
    ...
    <label style="display:inline-block;margin:8px 0 12px;">
        <input type="checkbox" id="tjobs-only-changes" <?php checked($only_changes, 1); ?>>
    ...
    <table class="widefat">
        ...
        <tbody>
            <tr>
                <td><code>[tjobs_list]</code></td>
                <td>Peruslistaus kaikista avoimista työpaikoista.</td>
            </tr>
            <tr>
                <td><code>[tjobs_by_country]</code></td>
                <td>
                    Maakohtainen ryhmittely (Suomi, Ruotsi, Kreikka, Italia) moderneilla korteilla + avoin hakemus -banneri lopussa.<br>
                    <code>[tjobs_by_country]</code> → Tumma teema (oletus)<br>
                    <code>[tjobs_by_country theme="dark"]</code> → Tumma teema<br>
                    <code>[tjobs_by_country theme="light"]</code> → Vaalea teema
                </td>
            </tr>
        </tbody>
    </table>
...
<script>
(function(){
    var cb = document.getElementById('tjobs-only-changes');
    if(!cb) return;
