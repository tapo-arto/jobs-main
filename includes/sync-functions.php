...
if ( ! function_exists( 'map_sync_feed' ) ) {
    ...
    if ($existing_query->have_posts()) {
        foreach ($existing_query->posts as $p_id) {
            ...
            if (!empty($meta_link)) {
                $existing_posts[map_clean_url($meta_link)] = $p_id;
            }
        }
    }
    ...
    foreach ($feed_items as $item) {
        ...
        foreach ($forbidden_titles as $bad_title) {
            if (!empty($bad_title) && stripos($title, $bad_title) !== false) {
                ...
                $open_application_lang = map_detect_open_application_language($title);
                if (!empty($form_url_skip) && !empty($open_application_lang)) {
                    $open_application_urls[$open_application_lang] = map_clean_url($form_url_skip);
                }
                break;
            }
        }
        ...
        $form_url = map_clean_url((!empty($form_tag[0]['data'])) ? trim($form_tag[0]['data']) : '');
        ...
        if (isset($existing_posts[$clean_link])) {
            ...
            update_post_meta($post_id, 'job_form_url', map_clean_url($form_url));
            ...
            if (!is_wp_error($new_post_id)) {
                update_post_meta($new_post_id, 'original_rss_link', map_clean_url($link));
                ...
                update_post_meta($new_post_id, 'job_form_url', map_clean_url($form_url));
                $added[] = $new_post_id;
            }
        }
    }
    ...
    if (!empty($open_application_urls)) {
        update_option('my_agg_open_application_urls', $open_application_urls, false);
        ...
        if (!empty($legacy_url)) {
            update_option('my_agg_open_application_url', $legacy_url, false);
        }
    }
