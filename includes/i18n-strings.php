 ...
 */
function tjobs_i18n( $key, $lang = null ) {
    if ( $lang === null ) {
        $lang = tjobs_get_current_lang();
}

$strings = tjobs_i18n_strings();
 ...
 */
function tjobs_get_current_lang() {
...
if ( function_exists( 'pll_current_language' ) ) {
    ...
    if ( $pll_lang ) {
        return tjobs_normalize_lang_code( $pll_lang );
    }
}

// 3. WPML
if ( defined( 'ICL_LANGUAGE_CODE' ) ) {
    return tjobs_normalize_lang_code( ICL_LANGUAGE_CODE );
}
 ...
 */
function tjobs_normalize_lang_code( $code ) {
 ...
 */
function tjobs_get_default_lang() {
    // Polylang
    if ( function_exists( 'pll_default_language' ) ) {
        $lang = pll_default_language();
        if ( $lang ) {
            return tjobs_normalize_lang_code( $lang );
        }
    }
 ...
 */
function tjobs_get_available_languages() {
    $languages = array();
    ...
    if ( function_exists( 'pll_languages_list' ) ) {
        ...
        if ( is_array( $pll_langs ) && ! empty( $pll_langs ) ) {
            foreach ( $pll_langs as $lang ) {
                $normalized = tjobs_normalize_lang_code( $lang );
    ...
    if ( function_exists( 'icl_get_languages' ) ) {
        ...
        if ( is_array( $wpml_langs ) && ! empty( $wpml_langs ) ) {
            foreach ( $wpml_langs as $lang ) {
                if ( isset( $lang['code'] ) ) {
                    $normalized = tjobs_normalize_lang_code( $lang['code'] );
                    if ( ! in_array( $normalized, $languages, true ) ) {
                        $languages[] = $normalized;
                    }
                }
 ...
 */
function tjobs_get_js_translations( $lang = null ) {
    if ( $lang === null ) {
        $lang = tjobs_get_current_lang();
    }
